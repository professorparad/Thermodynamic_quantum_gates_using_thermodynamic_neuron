...
\paragraph{HEOM depth scaling and MPS dimension scaling.}
...
For the tensor-network estimate, each reservoir was replaced by a finite
spin chain and the required MPS bond dimension was estimated from the
Schmidt spectra of the exact evolved state. For bath-chain lengths
...
estimated MPS bond dimension versus structured-bath chain length for the
direct subsystem and the Floquet-buffered subsystem.}
...
$|W_{\mathrm{drive}}|\simeq0.358182$ and has
$D_{\mathrm{buffer}}=0.855945$, giving a work-per-distinguishability proxy
$|W_{\mathrm{drive}}|/D_{\mathrm{buffer}}\simeq0.418464$. Thus the answer
...
$\Delta D=0.623372$ at drive-work proxy $|W|=0.539063$. The best
finite gain-per-drive-work point is
$A=0.55,\Omega=1.8,g_{SF}=0.08,\lambda=0.075,\gamma_c=3.0$, with
$D_{\mathrm{direct}}=0.594519$, $D_{\mathrm{buffer}}=0.883686$,
$\Delta D=0.289168$, and $|W|=1.40\times10^{-4}$. The very large
gain/work ratio at that point should be treated as a screened candidate
for refinement rather than a universal efficiency law.
...
drive-work proxy to gain output distinguishability. Marker size scales
with buffered final distinguishability.}
...
\end{equation}
The direct geometry is naturally an MPS if the bath chains are ordered
after the gate degrees of freedom. The buffered geometry is often better
as a TTN: the collector/modulator sits at the root, and the reference,
input, output, and modulator bath chains are separate branches. This
avoids forcing all bath entanglement through one artificial MPS bond.
